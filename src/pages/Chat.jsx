      ...
      if (!subscribedRef.current) {
         ...
         const changesChannel = supabase
            ...
            .on('postgres_changes', { schema: 'public', table: 'messaggi_chat', event: 'INSERT' }, async (payload) => {
               ...
               const { data } = await supabase.from('messaggi_chat').select(`*, utenti:id_utente(nickname, avatar_url)`).eq('id', payload.new.id).single()
   ...
   const loadMessages = async () => {
      try {
         const { data, error } = await supabase
            .from('messaggi_chat')
            .select(`*, utenti:id_utente(nickname, avatar_url)`)
                                    ...
                                    {REACTIONS.map(emoji => {
                                       ...
                                       return (
                                          <button
                                             ...
                                             className={`text-[10px] px-1.5 py-0.5 rounded-full transition-colors border ${count > 0
